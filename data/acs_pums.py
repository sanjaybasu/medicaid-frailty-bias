...
# IPUMS MSA-PUMA crosswalk URL (2023 OMB delineations × 2020 PUMAs)
MSA_PUMA_CROSSWALK_URL = (
    "https://usa.ipums.org/usa/resources/volii/MSA2023_PUMA2020_crosswalk.xlsx"
)
METRO_CACHE = DATA_DIR / "puma_metro_lookup.parquet"


def _build_metro_lookup() -> pd.DataFrame:
    """
    Build a PUMA → metro/nonmetro lookup from the IPUMS MSA-PUMA crosswalk.

    PUMAs appearing in the crosswalk intersect a metropolitan statistical area;
    those absent are classified as nonmetro. For PUMAs that partially overlap
    a metro area, we use the population-weighted majority classification.

    Returns DataFrame with columns: ST (int), PUMA (int), metro_status ('metro'/'nonmetro')
    """
    if METRO_CACHE.exists():
        return pd.read_parquet(METRO_CACHE)

    print("  Building PUMA → metro/nonmetro lookup from IPUMS crosswalk...")
    try:
        xwalk = pd.read_excel(MSA_PUMA_CROSSWALK_URL, engine='openpyxl')
    except Exception as e:
        print(f"  WARNING: Could not download IPUMS crosswalk ({e})")
        print("  Falling back to state-level urbanization proxy")
        return pd.DataFrame()

    # Crosswalk has columns like: State FIPS, PUMA, MSA Code, ...
    # Identify relevant columns
    cols = xwalk.columns.tolist()
    st_col = [c for c in cols if 'state' in c.lower() and 'fips' in c.lower()]
    puma_col = [c for c in cols if 'puma' in c.lower()]
    pop_col = [c for c in cols if 'pop' in c.lower()]

    if not st_col or not puma_col:
        # Try positional approach — IPUMS format: StateFIPS, PUMA, MSA/PMSA, ...
        st_col = [cols[0]]
        puma_col = [cols[1]]

    st_name = st_col[0]
    puma_name = puma_col[0]

    xwalk[st_name] = pd.to_numeric(xwalk[st_name], errors='coerce')
    xwalk[puma_name] = pd.to_numeric(xwalk[puma_name], errors='coerce')

    # All PUMAs in the crosswalk are metro (they intersect an MSA)
    metro_pumas = xwalk[[st_name, puma_name]].drop_duplicates()
    metro_pumas.columns = ['ST', 'PUMA']
    metro_pumas['metro_status'] = 'metro'

    metro_pumas.to_parquet(METRO_CACHE, index=False)
    print(f"  → {len(metro_pumas):,} metro PUMAs identified")
    return metro_pumas
...
def _filter_medicaid_adults(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Metro/nonmetro classification from PUMA
    if 'PUMA' in df.columns:
        metro_lookup = _build_metro_lookup()
        if not metro_lookup.empty:
            df['PUMA'] = pd.to_numeric(df['PUMA'], errors='coerce')
            df = df.merge(metro_lookup[['ST', 'PUMA', 'metro_status']],
                          on=['ST', 'PUMA'], how='left')
            df['metro_status'] = df['metro_status'].fillna('nonmetro')
        else:
            # Fallback: state-level urbanization proxy (ACS-derived)
            df['metro_status'] = 'unknown'
    else:
        df['metro_status'] = 'unknown'
